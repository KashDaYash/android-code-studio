...
class Kotlin(private val context: Context) : ILanguageServerInstaller {
  ...
  override fun isInstalled(): Boolean {
    // SERVERS_KOTLIN_DIR = .../acs/servers/kotlin/server
    // Process manager launches org.javacs.kt.MainKt from lib/*.jar — do not hardcode jar version.
    val libDir = File(Environment.SERVERS_KOTLIN_DIR, "lib")
    return libDir.isDirectory && (libDir.listFiles()?.any { it.name.endsWith(".jar") } == true)
  }
