...
class IDEApplication : TermuxApplication() {
  ...
  private fun extractLoggerRuntime() {
    try {
      ...
      // Keep a good AAR if already present
      if (targetFile.isFile && targetFile.length() > 64) {
        try {
          targetFile.inputStream().use { ins ->
            if (ins.read() == 0x50 && ins.read() == 0x4b) return
          }
        } catch (_: Exception) {
          /* re-extract below */
        }
      ...
      val candidates = listOf("logger-runtime.aar", "data/common/logger-runtime.aar")
      for (path in candidates) {
        try {
          ...
          if (targetFile.isFile && targetFile.length() > 64) {
            log.info(
                "Successfully extracted logger-runtime.aar from {} to {}",
                path,
          ...
          log.warn("logger-runtime.aar not in assets at {}: {}", path, e.message)
        }
      }
      log.warn(
          "logger-runtime.aar not packaged in this APK; project builds will skip log-sender injection"
      )
    } catch (e: Exception) {
