...
class GradleBuildService :
   ...
   *
   * If [logger-runtime.aar] is missing or invalid (common when the ACS APK was built without
   * packaging the logwire AAR), the AAR dependency is **skipped** so configure/sync still
   * succeeds. Desugaring remains enabled.
   */
  ...
  /** True if [file] looks like a usable AAR (non-empty ZIP). */
  private fun isValidLoggerAar(file: File): Boolean {
   ...
   * Tries assets root and data/common paths used across ACS variants.
   */
  private fun getLoggerRuntimeAar(): File {
    val aar = File(getLoggerPluginDir(), "logger-runtime.aar")
    if (isValidLoggerAar(aar)) return aar

    // Remove corrupt/empty file so we can re-extract
    if (aar.exists()) {
      aar.delete()
    }
    ...
    val assetCandidates =
        listOf(
            "logger-runtime.aar",
            ToolsManager.getCommonAsset("logger-runtime.aar"), // data/common/logger-runtime.aar
        )
    for (asset in assetCandidates) {
      try {
        if (
            ResourceUtils.copyFileFromAssets(asset, aar.absolutePath) && isValidLoggerAar(aar)
        ) {
          log.info("Extracted logger-runtime.aar from assets path: {}", asset)
          return aar
        }
      } catch (e: Exception) {
        log.warn("Could not copy logger AAR from {}: {}", asset, e.message)
      }
    }
    log.error("Failed to extract a valid logger-runtime.aar from assets")
    return aar
  }
